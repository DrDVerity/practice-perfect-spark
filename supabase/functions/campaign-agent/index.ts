...
serve(async (req) => {
  ...
  try {
    const { messages, campaignName, systemPrompt, practiceReport } = await req.json();
    ...
    const systemMessage = [
      ...
      "Provide specific, actionable advice. Suggest content ideas, channel strategies, scheduling recommendations, and audience targeting tips. Be concise but thorough.",
    ]
      .filter(Boolean)
      .join("\n\n");
